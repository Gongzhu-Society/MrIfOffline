...
### MrNN
以下是两个MrNN对战两个MrIf 1024x2 盘的结果, 结果不尽如人意, 它模仿Greed这么好, 为什么效果这么差呢? 但是好在他很快, 出每张牌只需要大约 72/(1024x2)/26=1.3ms
```
...
```
发现是之前把训练集当测试集输进去了, 实际测试正确率只有(47.7,71.6,79.7,85.7), 所以输 MrIf 50分现在是可以理解的了
