...
def train_guesser(pv_net, guesser_net,dev_train_num=0,args={},dev_bench_num=0):
    ...
    if names[0] in {"Guessing_net_1"}:
        optimizer = optim.Adam(guesser_net.parameters(), lr=args['guesser_lr'], eps=1e-07, weight_decay=1e-4,
                               amsgrad=False)
    else:
        optimizer=optim.Adam(guesser_net.parameters(),lr=args['guesser_lr'],betas=(0.3,0.999),eps=1e-07,weight_decay=1e-4,amsgrad=False)
    log("optimizer: %s"%(optimizer.__dict__['defaults'],))
...
def main_for_guesser(args):
    ...
    dev_train=0
    start_from=args['gs_start_from'] # or a path to netpara file
    if args['pv_net'] in {'PV_NET_4'}:
        pv_net = PV_NET_4()  # RES_NET_18()#PV_NET_2()
    else:
        pv_net = PV_NET_3()
